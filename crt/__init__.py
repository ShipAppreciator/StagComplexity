...
class End(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if player.crt_Q3 == 47:
            player.questions_correct += 1
        player.participant.vars['crt_payoff'] = cu(player.questions_correct * 0.50)
        player.participant.payoff = (
            player.participant.vars.get('stag_payoff', cu(0)) +
            player.participant.vars.get('bret_payoff', cu(0)) +
            player.participant.vars['crt_payoff']
        )
